...
function PizzaBlock({ id, name, price, imageUrl, sizes, types }) {
    ...
    const typeDoughNames = ['тонкое', 'традиционное']
            ...
            <div className="pizza-block">
                ...
                <div className="pizza-block__selector">
                    <ul>
                        {
                            types.map((index) => (
                                <li key={index} onClick={() => setDoughType(index)} className={index === doughType ? 'active' : ''}>{typeDoughNames[index]}</li>
                            ))
                        }
                    </ul>
                    <ul>
                        {
                            sizes.map((size, index) => (
                                <li key={size} onClick={() => setPizzaSize(index)} className={pizzaSize === index ? 'active' : 0}>{size} см.</li>
                            ))
                        }
                    </ul>
                </div>
